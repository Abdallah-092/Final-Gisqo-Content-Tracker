...
const AdminHub: React.FC<AdminHubProps> = ({ 
  ...
  const stats = useMemo(() => {
    ...
    const byType = {
      ...
      // Fix: 'Other' comparison is now valid following ContentType correction in types.ts
    ...
    const performance = creatorsList.map(c => {
      ...
      // Calculate period goals
      let periodGoal = settings.dailyGoal;
      if (reportPeriod === 'weekly') periodGoal = settings.dailyGoal * 5; // 5 working days
      if (reportPeriod === 'monthly') periodGoal = settings.dailyGoal * 22; // approx 22 working days
  ...
  const removeClient = (id: string) => {
    if (confirm("Are you sure you want to remove this client? Historical entries will remain but this client won't be selectable for new logs.")) {
                 ...
                 {clients.map(client => (
                   <div key={client.id} className="bg-slate-900/40 p-8 rounded-[2.5rem] border border-slate-800 flex justify-between items-center group hover:bg-slate-800 transition-all">
                      ...
                      <div className="flex space-x-2">
                        ...
                        <button onClick={() => removeClient(client.id)} className="p-3 bg-slate-800 rounded-xl hover:text-red-500 transition-colors" title="Delete Client">
